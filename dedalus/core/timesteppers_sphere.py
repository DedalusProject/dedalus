...
class MultistepIMEX:
    ...
    * Our coefficients are related to those used by Wang as:
    ...
    def step(self, dt, state_vector, solver, NL, LU):
        ...
        for i, subproblem in enumerate(solver.subproblems):
            ...
            MX0.data[i] = M @ state_vector.data[i]
            LX0.data[i] = L @ state_vector.data[i]
            np.copyto(F0.data[i],NL.data[i])

            # Build RHS
            RHS.data[i] *= 0
            for j in range(1, len(c)):
                RHS.data[i] += c[j] *  F[j-1].data[i]
            for j in range(1, len(a)):
                RHS.data[i] -= a[j] * MX[j-1].data[i]
            for j in range(1, len(b)):
                RHS.data[i] -= b[j] * LX[j-1].data[i]

            # Solve
            if STORE_LU:
                if update_LHS:
                    LU[i] = linalg.splu(P.tocsc(), permc_spec=PERMC_SPEC)
                pLHS = LU[i]
                state_vector.data[i] = pLHS.solve(RHS.data[i])
            else:
                state_vector.data[i] = linalg.spsolve(P,RHS.data[i], use_umfpack=USE_UMFPACK, permc_spec=PERMC_SPEC)
