...
class CylindricalComponent(LinearOperator):
    ...
    def __init__(self, operand, index=0, out=None):
        ...
        self.index = index
        self.coordsys = operand.tensorsig[self.index]
        if not isinstance(self.coordsys, coords.CylindricalCoordinates):
            raise ValueError("Can only take the CylindricalComponent of a CylindricalCoordinate vector")
        super().__init__(operand, out=out)
        # LinearOperator requirements
        self.operand = operand
        # FutureField requirements
...
class VerticalComponent(CylindricalComponent, metaclass=MultiClass):
    ...
    def new_operand(self, operand, **kw):
        return VerticalComponent(operand, self.index, **kw)


class HorizontalComponent(CylindricalComponent, metaclass=MultiClass):
    ...
    def __init__(self, operand, index=0, out=None):
        super().__init__(operand, index=index, out=out)
        tensorsig = operand.tensorsig
        polar_coordsys = tensorsig[index].polar_coordsys
        self.tensorsig = tuple( tensorsig[:index] + (polar_coordsys,) + tensorsig[index+1:] )

    def new_operand(self, operand, **kw):
        return HorizontalComponent(operand, self.index, **kw)
