...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_scalar_prod_vector(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
    ...
    k_out = k_out_shell(k_ncc, u.domain.bases[0].k)
    vars = [u]
    w0 = f * u
    w1 = w0.reinitialize(ncc=True, ncc_vars=vars)
    problem = problems.LBVP(vars)
...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_scalar_prod_tensor(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
    ...
    k_out = k_out_shell(k_ncc, T.domain.bases[0].k)
    vars = [T]
    w0 = f * T
    w1 = w0.reinitialize(ncc=True, ncc_vars=vars)
    problem = problems.LBVP(vars)
...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_vector_prod_scalar(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
    ...
    k_out = k_out_shell(u.domain.bases[0].k, g.domain.bases[0].k)
    vars = [g]
    w0 = u * g
    w1 = w0.reinitialize(ncc=True, ncc_vars=vars)
    problem = problems.LBVP(vars)
...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_vector_prod_vector(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
    ...
    k_out = k_out_shell(u.domain.bases[0].k, v.domain.bases[0].k)
    vars = [v]
    w0 = u * v
    w1 = w0.reinitialize(ncc=True, ncc_vars=vars)
    problem = problems.LBVP(vars)
...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_vector_dot_vector(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_vector_dot_tensor(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
    ...
    u = operators.Gradient(f, c).evaluate()
    T = operators.Gradient(operators.Gradient(g, c), c).evaluate()      
    k_out = k_out_shell(u.domain.bases[0].k, T.domain.bases[0].k)
    vars = [T]
    w0 = dot(u, T)
    w1 = w0.reinitialize(ncc=True, ncc_vars=vars)
    problem = problems.LBVP(vars)
...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_tensor_prod_scalar(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
    ...
    k_out = k_out_shell(T.domain.bases[0].k, g.domain.bases[0].k)
    vars = [g]
    w0 = T * g
    w1 = w0.reinitialize(ncc=True, ncc_vars=vars)
    problem = problems.LBVP(vars)
...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_tensor_dot_vector(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
    ...
    T = operators.Gradient(operators.Gradient(f, c), c).evaluate()
    u = operators.Gradient(g, c).evaluate()
    k_out = k_out_shell(T.domain.bases[0].k, u.domain.bases[0].k)
    vars = [u]
    w0 = dot(T, u)
    w1 = w0.reinitialize(ncc=True, ncc_vars=vars)
    problem = problems.LBVP(vars)
...
@pytest.mark.parametrize('dealias', dealias_range)
@pytest.mark.parametrize('dtype', dtype_range)
def test_shell2D_tensor_dot_tensor(Nphi, Ntheta, Nr, alpha, k_ncc, k_arg, dealias, dtype):
    c, d, b, phi, theta, r, x, y, z = build_shell(Nphi, Ntheta, Nr, alpha, dealias=dealias, dtype=dtype)
    b_ncc = b.meridional_basis.clone_with(k=k_ncc)
    b_arg = b.clone_with(k=k_arg)
    f = field.Field(dist=d, bases=(b_ncc,), dtype=dtype)
