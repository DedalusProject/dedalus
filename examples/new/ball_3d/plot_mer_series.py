...
def pcolormesh(mfig, k, taskname, dset, index, first, plot_list, write_num):
    ...
    if first:
      if k == 0:
          # Create axes
          i, j = divmod(k, ncols)
          mfig.paxes = mfig.add_axes(i, j, [0., 0., 1., 0.93])
    ...
    th = x[xorder]
    th_mid = (th[1:] + th[:-1])/2
    th = np.concatenate(([0], th_mid, [np.pi]))
    r = y[yorder]
    r = np.concatenate(([0], r))
    yv = r[:,None]*np.cos(th[None,:])
    if k == 0:
        xv = r[:,None]*np.sin(th[None,:])
    elif k == 1:
        xv = -r[:,None]*np.sin(th[None,:])
#    xv, yv = get_vertex_arrays(x, y)
    ...
    if first:
      # Plot
      plot_list.append(mfig.paxes.pcolormesh(xv, yv, datsort, cmap=cmap, zorder=1))
      if k==0:
          mfig.paxes.axis(build_limits(mfig.paxes, xv, yv, ypad=0.0, square=False))
          mfig.paxes.tick_params(length=0, width=0)
          ...
          mfig.paxes.set_ylabel('z')
          mfig.paxes.set_xlabel('x')

    else:
      #plot_list[k].set_array(np.ravel(datsort[:-1,:-1]))
      plot_list[k].set_array(np.ravel(datsort))

    # consistent color table across both slices
    if k==0:
        mfig.data_min = datsort.min()
        mfig.data_max = datsort.max()
    elif k==1:
        mfig.data_min = min(mfig.data_min, datsort.min())
        mfig.data_max = max(mfig.data_max, datsort.max())
        plot_list[0].set_clim(mfig.data_min, mfig.data_max)
        plot_list[1].set_clim(mfig.data_min, mfig.data_max)

def build_limits(axis, x, y, xpad=0., ypad=0., square=False):

    xmin, xmax, ymin, ymax = -x.max(), x.max(), y.min(), y.max()
    dx = xmax - xmin
    dy = ymax - ymin
    x0 = xmin - xpad*dx
    x1 = xmax + xpad*dx
