...
def main(filename, start, count, output):
    ...
    with h5py.File(filename, mode='r') as file:
        ...
        for index in range(start, start+count):
            data_slices = (index, slice(None), slice(None), slice(None))
            data = dset[data_slices]
            for i in range(2):
                if index == start:
                    pcms.append( paxes[i].pcolormesh(x, y, data[i], cmap=cmap) )
                    ...
                    pcms[i].set_array(np.ravel(data[i]))
                    pcms[i].set_clim(data[i].min(), data[i].max())
                    cbaxes[i].xaxis.set_ticks_position('top')
                    cbaxes[i].xaxis.set_label_position('top')
