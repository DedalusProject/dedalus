...
# Options
fnames = ['u', 'v', 'w', 'T']
sli = [slice(0,1), slice(None), slice(None)]
xstr = 'y'
ystr = 'z'
cmapname = 'Spectral_r'
even_scale = True
...
for j, (fname, field) in enumerate(zip(fnames, fields)):
    imax = image_axes[j]
    cbax = cbar_axes[j]
    data = field[0][sli].squeeze().T
    images.append(add_image(fig, imax, cbax, x, y, data))
...
for i in range(1, t.size):
    for j, field in enumerate(fields):
        data = field[i][sli].squeeze().T
        update_image(images[j], data)
